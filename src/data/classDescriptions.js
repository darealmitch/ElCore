export const classDescriptions = [
    /*Elsword*/
    {
        characterId: "elsword",
        pathName: "Knight Emperor",
        pathNameFr: "Chevalier Royal",
        className: "Knight Emperor",
        classNameFr: "Chevalier Royal",
        jobStage: "job3",
        title: "",
        path: ["Chevalier de l'épée", "Haut Chevalier", "Chevalier Royal"],
        presentation: "Combattant agressif avant d'amélioré ses techniques de corps à corps",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Facile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "elsword",
        pathName: "Rune Master",
        pathNameFr: "Maître des runes",
        className: "Rune Master",
        classNameFr: "Maître des runes",
        jobStage: "job3",
        title: "Elsword 2e voie : Maître des runes",
        path: ["Chevalier-Mage", "Chevalier Runique", "Maître des runes"],
        presentation: "Combattant des flammes utilisant l'épée et la magie runique",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Facile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "elsword",
        pathName: "Immortal",
        pathNameFr: "Immortel",
        className: "Immortal",
        classNameFr: "Immortel",
        jobStage: "job3",
        title: "Elsword 3e voie : Immortel",
        path: ["Chevalier de Cornwell", "Invocateur d'épées", "Immortel"],
        presentation:
            "Des possibilités infinies s'offrent à cet épéiste, qui maîtrise les ténèbres et repousse ses propres limites. Grâce à cette spécialisation, tu utiliseras tes deux épées pour acculer l'ennemi sans relâche.",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Facile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "elsword",
        pathName: "Genesis",
        pathNameFr: "Séphiroth",
        className: "Genesis",
        classNameFr: "Séphiroth",
        jobStage: "job3",
        title: " Elsword 'e voie : Séphiroth",
        path: ["Bénitier", "Templier", "Séphiroth"],
        presentation:
            "Un chevalier qui intègre le pouvoir de la résonance d'El dans son épée pour anéantir ses ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },

    /*Rena*/
    {
        characterId: "rena",
        pathName: "Anemos",
        pathNameFr: "Anemos",
        className: "Anemos",
        classNameFr: "Anemos",
        jobStage: "job3",
        title: "Rena 1er voie : Anemos",
        path: ["Guerrière archère", "Voyageuse du vent", "Anemos"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "rena",
        pathName: "Daybreaker",
        pathNameFr: "Aube",
        className: "Daybreaker",
        classNameFr: "Aube",
        jobStage: "job3",
        title: "Rena 2e voie : Aube",
        path: ["Chasseresse", "Maîtresse-Archère", "Aube"],
        presentation: "Cette meneuse des nobles Elfes bénéficie de la protection du monde des esprits. " +
            "C'est une Elfe guerrière dont les flèches magiques renforcées par le lien qui l'unit aux esprits transpercent rapidement la défense ennemie.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Facile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "rena",
        pathName: "Twilight",
        pathNameFr: "Crépuscule",
        className: "Twilight",
        classNameFr: "Crépuscule",
        jobStage: "job3",
        title: "Rena 3e voie : Crépuscule",
        path: ["Guerrière d'Erendil", "Ombre sylvestre", "Crépuscule"],
        presentation:
            "Cette élégante gardienne de la nuit dirige les ombres de la nature. Elle fond sur ses adversaires avec son arc et son épée, armes imprégnées de la puissance des esprits.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "rena",
        pathName: "",
        pathNameFr: "Prophétesse",
        className: "",
        classNameFr: "Prophétesse",
        jobStage: "job3",
        title: "La 4e voie de Rena va enfin sortir : écoutez l'histoire racontée par la Prophétesse.",
        path: ["Poétesse archère", "Tisseuse d'histoires", "Prophétesse"],
        presentation:
            "Communiquez avec les esprits et mettez leur pouvoir à profit pour aider et guider les humains en détresse.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },

    /*Aisha*/
    {
        characterId: "aisha",
        pathName: "Aether Sage",
        pathNameFr: "Étheriste",
        className: "Aether Sage",
        classNameFr: "Étheriste",
        jobStage: "job3",
        title: "Aisha 1er voie : Étheriste",
        path: ["Archimage", "Maîtresse des éléments", "Étheriste"],
        presentation: "",
        stats: {
            speed: "Lente",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "aisha",
        pathName: "Oz Sorcerer",
        pathNameFr: "Magicienne d'Oz",
        className: "Oz Sorcerer",
        classNameFr: "Magicienne d'Oz",
        jobStage: "job3",
        title: "Aisha 2e voie : Magicienne d'Oz",
        path: ["Magicienne de l'occulte", "Princesse des âmes", "Magicienne d'Oz"],
        presentation: "La Magicienne d'Oz est la maîtresse de la magie noire reconnue par le Dieu démoniaque en personne. Sa connaissance approfondie de la magie noire lui permet de mener ses ennemis à la perdition.",
        stats: {
            speed: "Lent",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "aisha",
        pathName: "",
        pathNameFr: "Métamorphia",
        className: "",
        classNameFr: "Métamorphia",
        jobStage: "job3",
        title: "Aisha 3e voie : Métamorphia",
        path: [],
        presentation:
            "La Métamorphia est une puissante sorcière, qui fait de tout l'espace-temps son terrain de jeu. Elle écrase ses ennemis avec de violentes explosions d'énergie et une maîtrise du corps envoûtante.",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "aisha",
        pathName: "Lord Azoth",
        pathNameFr: "Druidesse d'Azoth\n",
        className: "Lord Azoth",
        classNameFr: "Druidesse d'Azoth\n",
        jobStage: "job3",
        title: "Aisha 4e voie : Druidesse d'Azoth\n",
        path: ["Magicienne de génie","Alchimiste mystique","Druidesse d'Azoth"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },

    /*Raven*/
    {
        characterId: "raven",
        pathName: "Furious Blade",
        pathNameFr: "Lame furieuse",
        className: "Furious Blade",
        classNameFr: "Lame furieuse",
        jobStage: "job3",
        title: "Raven 1er voie : Lame furieuse",
        path: ["Porteur d'épée", "Maître de l'épée","Lame furieuse" ],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "raven",
        pathName: "Rage Hearts",
        pathNameFr: "Cœur enragé",
        className: "Rage Hearts",
        classNameFr: "Cœur enragé",
        jobStage: "job3",
        title: "Raven 2e voie : Cœur enragé",
        path: ["Porteur de lame", "Poing de Feu","Cœur enragé" ],
        presentation: "Ce combattant a transcendé les limites de son enveloppe corporelle humaine grâce à la coexistence de sa force nasod. " +
            "Il use de la force explosive dévastatrice du bras nasod pour pulvériser ses adversaires.",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "raven",
        pathName: "Imperator Nova",
        pathNameFr: "Imperator Nova",
        className: "Imperator Nova",
        classNameFr: "Imperator nova",
        jobStage: "job3",
        title: "Raven 3e voie : Imperator nova",
        path: ["Maître de la destinée", "Commandant Vétéran", "Imperator nova"],
        presentation:
            "Ce stratège garde la tête froide en se servant de lui-même comme d'une arme. La spécialisation convertit la puissante énergie du cœur de la flamme en feu qui brûle ses ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "raven",
        pathName: "Revenant",
        pathNameFr: "Âme fatale",
        className: "Revenant",
        classNameFr: "Âme fatale",
        jobStage: "job3",
        title: "Raven 4e voie : Âme fatale",
        path: ["Venimophore", "Métalame", "Âme fatale"],
        presentation: "Cette classe plante les graines d'une plante parasitaire dans ses adversaires pour les détruire de l'intérieur.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    /*Eve*/
    {
        characterId: "eve",
        pathName: "Code: Ultimate",
        pathNameFr: "Nasod ultime",
        className: "Code: Ultimate",
        classNameFr: "Nasod ultime",
        jobStage: "job3",
        title: "Ève 1er voie : Nasod ultime",
        path: ["Vengeresse nasod", "Némésis nasod","Nasod ultime"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "eve",
        pathName: "Code: Esencia",
        pathNameFr: "Nasod essentielle",
        className: "Code: Esencia",
        classNameFr: "Nasod essentielle",
        jobStage: "job3",
        title: "Ève 2e voie : Nasod essentielle",
        path: ["Espérence Nasod", "Impératrice Nasod","Nasod essentielle"],
        presentation: "La très miséricordieuse Impératrice nasod blanc neige commande ses fidèles familiers nasods pour terrasser les ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "eve",
        pathName: "Code: Sariel",
        pathNameFr: "Nasod Sariel",
        className: "Code: Sariel",
        classNameFr: "Nasod Sariel",
        jobStage: "job3",
        title: "Ève 3e voie : Nasod Sariel",
        path: ["Nasod Elektra","Séraphin Nasod", "Nasod Sariel"],
        presentation: "Voici une reine des Nasods rayonnante d'éclairs. Elle anéantit ses adversaires avec de l'énergie photonique émise à la vitesse lumière.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Facile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "eve",
        pathName: "",
        pathNameFr: "Nasod Antithesa",
        className: "",
        classNameFr: "Nasod Antithesa",
        jobStage: "job3",
        title: "Ève 4e voie : Nasod Antithesa",
        path: ["Nasod Incognita", "Nasod Absoluta", "Nasod Antithesa"],
        presentation: "Ève génère les codes de manœuvre aérienne qui avaient été perdus et regagne en mobilité.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },

    /*Chung*/
    {
        characterId: "chung",
        pathName: "Comet Crusader",
        pathNameFr: "Chevalier croisé cométaire",
        className: "Comet Crusader",
        classNameFr: "Chevalier croisé cométaire",
        jobStage: "job3",
        title: "Chung 1er voie : Chevalier croisé cométaire",
        path: ["Gardien colérique", "Paladin adamantium","Chevalier croisé cométaire"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Facile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "chung",
        pathName: "Fatal Phantom",
        pathNameFr: "Fantôme fatal",
        className: "Fatal Phantom",
        classNameFr: "Fantôme fatal",
        jobStage: "job3",
        title: "Chung 2e voie : Fantôme fatal",
        path: ["Gardien tireur", "Flingueur","Fantôme fatal"],
        presentation: "Ce tireur submerge les ennemis sous un déluge de projectiles à une vitesse fulgurante." +
            " En un clin d'œil, il peut décharger une véritable tempête de projectiles pour triompher de ses ennemis.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "chung",
        pathName: "",
        pathNameFr: "Centurion",
        className: "",
        classNameFr: "Centurion",
        jobStage: "job3",
        title: "Chung 3e voie : Centurion",
        path: ["Gardien artilleur","Canonnier de commando","Centurion"],
        presentation: "Cet artilleur est comme une forteresse ambulante. " +
            "Grâce à ses recherches, il a pu transcender les limites de la physique. " +
            "Les membres de cette classe sont capables de tirer d'innombrables obus depuis le portail sur le champ de bataille.",
        stats: {
            speed: "Lent",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "chung",
        pathName: "",
        pathNameFr: "Dius Aer",
        className: "",
        classNameFr: "Dius Aer",
        jobStage: "job3",
        title: "« J'offrirai ma protection à qui en cherche une. » La 4e voie de Chung s'ouvre.",
        path: ["Gardien des cœurs", "Phanès sacré", "Dius Aer"],
        presentation: "La pierre de protection soutient tes alliés. " +
            "Ton armure ultralégère, Freiturnier, te permet d'attaquer plus rapidement.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },

    /*Elesis*/
    {
        characterId: "elesis",
        pathName: "Empire Sword",
        pathNameFr: "Épée de l'empire",
        className: "Empire Sword",
        classNameFr: "Épée de l'empire",
        jobStage: "job3",
        title: "Elesis 1er voie : Épée de l'empire",
        path: ["Maître-escrimeur", "Grand Maître", "Épée de l'empire"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "elesis",
        pathName: "Flamme Lord",
        pathNameFr: "Maîtresse des flammes",
        className: "Flamme Lord",
        classNameFr: "Maîtresse des flammes",
        jobStage: "job3",
        title: "Elesis 2e voie : Maîtresse des flammes",
        path: ["Chevalier Pyromane", "Cœur de Flammes", "Maîtresse des Flammes"],
        presentation: "L'incarnation du feu enveloppée de flammes. " +
            "La Maîtresse des flammes est elle-même devenue une flamme qui réduit tout en cendre d'une seule flammèche.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "elesis",
        pathName: "",
        pathNameFr: "Reine sanglante",
        className: "",
        classNameFr: "Reine sanglante",
        jobStage: "job3",
        title: "Elesis 3e voie : Reine sanglante",
        path: [],
        presentation: "Cette cavalière de la mort est assoiffée de sang. " +
            "La Reine sanglante possède une force destructrice issue de l'art occulte de manier l'épée, qui lui permet d'exterminer tous ses ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Facile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "elesis",
        pathName: "Adrestia",
        pathNameFr: "Adrestia",
        className: "Adrestia",
        classNameFr: "Adrestia",
        jobStage: "job3",
        title: "Repoussant sa destinée oppressante, Elesis, l'épée à la main, prend un nouveau chemin pour protéger réellement Élios.",
        path: ["Chevalier fauconnier", "Patrona", "Adrestia"],
        presentation: "Armée de l'épée sacrée Divinus Manus, tu te bats n'importe où et contre n'importe quelle horde pour rester fidèle à ton noble objectif.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },

    /*Ara*/
    {
        characterId: "ara",
        pathName: "Apsara",
        pathNameFr: "Apsara",
        className: "Apsara",
        classNameFr: "Apsara",
        jobStage: "job3",
        title: "Ara 1er voie : Apsara",
        path: ["Petite Hsien", "Sakra Devanam","Apsara"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "ara",
        pathName: "Devi",
        pathNameFr: "Devi",
        className: "Devi",
        classNameFr: "Devi",
        jobStage: "job3",
        title: "Ara 2e voie : Devi",
        path: ["Diablotin", "Yama-raj","Devi"],
        presentation: "Une meneuse d'outre-tombe qui sème la mort. " +
            "Elle déploie un flot ininterrompu de technique énergétique fulminante pour anéantir ses ennemis.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "ara",
        pathName: "Shakti",
        pathNameFr: "Shakti",
        className: "Shakti",
        classNameFr: "Shakti",
        jobStage: "job3",
        title: "Ara 3e voie : Shakti",
        path: ["Petit Spectre", "Asura", "Shakti"],
        presentation:
            "Voici la première déesse renarde, créée par le lien avec une divinité. Elle triomphe de ses adversaires en usant de puissantes techniques d'exorcisme de l'ombre tout en s'alliant à Eun, la Gumiho.",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Facile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "ara",
        pathName: "Surya",
        pathNameFr: "Surya",
        className: "Surya",
        classNameFr: "Surya",
        jobStage: "job3",
        title: "L'as de la création Ara obtient sa 4e voie !",
        path: ["Bienfaitrice", "Yang Yan", "Surya"],
        presentation: "Cette classe s'appuie sur les esprits animaux pour bénéficier de divers pouvoirs afin d'attaquer les adversaires ou soutenir ses alliés.",
        stats: {
            speed: "Rapide",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },

    /*Add*/
    {
        characterId: "add",
        pathName: "Doom Bringer",
        pathNameFr: "Calamité",
        className: "Doom Bringer",
        classNameFr: "Calamité",
        jobStage: "job3",
        title: "Add 1er voie : Calamité",
        path: ["Traqueur psychique", "Psychopathe", "Calamité"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "add",
        pathName: "",
        pathNameFr: "Dominateur",
        className: "",
        classNameFr: "Dominateur",
        jobStage: "job3",
        title: "Add 2e voie : Dominateur",
        path: ["Maître de l'IA", "Machiavel", "Dominateur"],
        presentation: "Ce savant fou a façonné un monde suprême à l'image de ses idéaux. " +
            "Il se sert de son système PCD imprévisible pour prendre le contrôle du champ de bataille.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "add",
        pathName: "",
        pathNameFr: "Paradoxe",
        className: "",
        classNameFr: "Paradoxe",
        jobStage: "job3",
        title: "Add 3e voie : Paradoxe",
        path: ["Voyageur Temporel", "Seigneur du Temps", "Paradoxe"],
        presentation: "Ce diable du Paradoxe est pétri de contradictions multiples et erre dans l'espace-temps. " +
            "Doté de compétences largement supérieures à la pointe de la science, il fait s'effondrer l'espace-temps avec brutalité.",
        stats: {
            speed: "Rapide",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "add",
        pathName: "Overmind",
        pathNameFr: "Subjugueur",
        className: "Overmind",
        classNameFr: "Subjugueur",
        jobStage: "job3",
        title: "Add 4e voie : Subjugueur",
        path: ["Traceur d'impulsions", "Décomposeur", "Subjugueur"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },

    /*Lu/Ciel*/
    {
        characterId: "luciel",
        pathName: "Catastrophe",
        pathNameFr: "Catastrophe",
        className: "Catastrophe",
        classNameFr: "Catastrophe",
        jobStage: "job3",
        title: "Lu/Ciel 1er voie : Catastrophe",
        path: ["Chiliarque", "Ange de la Mort", "Catastrophe"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "luciel",
        pathName: "",
        pathNameFr: "Innocence",
        className: "Innocent",
        classNameFr: "Innocence",
        jobStage: "job3",
        title: "Lu/Ciel 2e voie : Innocence",
        path: ["Garde Royal", "Noble", "Innocence"],
        presentation: "Tels un javelot et un bouclier qui se protègent mutuellement, Lu et Ciel ont transcendé leur puissance passée et règnent désormais sur les abîmes. " +
            "Ils se servent d'esprits malins des abîmes et d'armes antiques pour vaincre leurs ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }

    },
    {
        characterId: "luciel",
        pathName: "",
        pathNameFr: "Diangelion",
        className: "",
        classNameFr: "Diangelion",
        jobStage: "job3",
        title: "Lu/Ciel 3e voie : Diangelion",
        path: ["Diabla","Demonio", "Diangelion"],
        presentation: "Voici la reine démoniaque de la destruction et son fidèle serviteur. " +
            "Enveloppés d'énergie démoniaque noire comme le jais, ils exécutent chaque ennemi sans hésitation.",
        stats: {
            speed: "Rapide",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }

    },
    {
        characterId: "luciel",
        pathName: "",
        pathNameFr: "Demersio",
        className: "",
        classNameFr: "Demersio",
        jobStage: "job3",
        title: "La 4e voie de Lu/Ciel est enfin publiée ! La souveraine sombrant dans le vide et son sauveur impitoyable",
        path: ["Isolatesse", "Tristesso", "Demersio"],
        presentation: "Id et Ego se sont manifestés dans l'abysse. " +
            "En fonction de la situation pendant un combat, cette classe peut choisir de jouer l'un de ces rôles.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            PictureType: "/Images/Combat/Magical_Class.png",
            AttackType: "Attaque Magique",
        }
    },
    /*Rose*/
    {
        characterId: "rose",
        pathName: "Tempest Burster",
        pathNameFr: "Tempête furieuse",
        className: "Tempest Burster",
        classNameFr: "Tempête furieuse",
        jobStage: "job3",
        title: "Rose 1er voie : Tempête furieuse",
        path: ["Grand Calibre","Avant-Garde", "Tempête furieuse"],
        presentation: "",
        stats: {
            speed: "Lente",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "rose",
        pathName: "Black Massacre",
        pathNameFr: "Veuve noire",
        className: "Black Massacre",
        classNameFr: "Veuve noire",
        jobStage: "job3",
        title: "Rose 2e voie : Veuve noire",
        path: ["Sanguinaire","Rose pourpre", "Veuve Noire"],
        presentation: "Cette vagabonde rouge sang se consacre corps et âme au massacre. " +
            "Elle entrave ses ennemis à l'aide de la pistochaîne pour leur asséner un coup mortel.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "rose",
        pathName: "",
        pathNameFr: "Minerve",
        className: "",
        classNameFr: "Minerve",
        jobStage: "job3",
        title: "Rose 3e voie : Minerve",
        path: ["Valkyrie", "Freyja", "Minerve"],
        presentation: "Cette maîtresse dans l'art des combats aériens use de sa formidable mobilité. " +
            "Elle change le cours du combat en un rien de temps et triomphe de ses ennemis avec diverses grenades et techniques de combat à distance.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "rose",
        pathName: "Prime Operator",
        pathNameFr: "Prime Opératrice",
        className: "Prime Operator",
        classNameFr: "Prime Opératrice",
        jobStage: "job3",
        title: "Rose 4e voie : Prime Opératrice",
        path: ["Cœur d'Acier", "Optimus", "Prime Opératrice"],
        presentation: "Voici l'inventrice de la légion mécanique contrôlée par satellite. " +
            "Elle commande des machines ultra sophistiquées de sa propre création.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }

    },

    /*Ain*/
    {
        characterId: "ain",
        pathName: "Richter",
        pathNameFr: "Arbiter",
        className: "Richter",
        classNameFr: "Arbiter",
        jobStage: "job3",
        title: "Ain 1er voie : Arbiter",
        path: ["Exactor", "Prodigium", "Arbiter"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "ain",
        pathName: "Bluhen",
        pathNameFr: "Florere",
        className: "Bluhen",
        classNameFr: "Florere",
        jobStage: "job3",
        title: "Ain 2e voie : Florere",
        path: ["Aptare", "Vigere", "Florere"],
        presentation: "Ce meneur d'émotions se développe au même rythme que les humains qui l'entourent. " +
            "Il se sert de serments pour protéger ses alliés et terrasser ses ennemis.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Difficile",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "ain",
        pathName: "Herrscher",
        pathNameFr: "Regnator",
        className: "Herrscher",
        classNameFr: "Regnator",
        jobStage: "job3",
        title: "Ain 3e voie : Regnator",
        path: ["Viator","Apostata","Regnator"],
        presentation: "Ce maître du vide régénéré attire tout vers le néant. " +
            "Il met ses ennemis hors d'état de nuire par un chaos infini et anéantit tout en usant de la force du vide.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }

    },
    {
        characterId: "ain",
        pathName: "",
        pathNameFr: "Sacratus",
        className: "",
        classNameFr: "Sacratus",
        jobStage: "job3",
        title: "Le serviteur errant guette vainement sa déesse : mise à jour pour la 4e voie d'Ain",
        path: ["Adoleo", "Captura", "Sacratus"],
        presentation: "Pendant qu'Ain absorbe la force d'Eldrit de tout un chacun, son savoir-faire prend un contour plus défini. " +
            "Dans cette classe, Ain combat en déployant une vaste palette d'astuces redoutablement efficaces.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },

    /*Laby*/
    {
        characterId: "laby",
        pathName: "Eternity Winner",
        pathNameFr: "Championne Éonienne",
        className: "Eternity Winner",
        classNameFr: "Championne Éonienne",
        jobStage: "job3",
        title: "Laby 1er voie : Championne Éonienne",
        path: ["Bébé Étincelle", "Bébé Cyclone", "Championne Éonienne"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "laby",
        pathName: "Radiant Soul",
        pathNameFr: "Lumièle",
        className: "Radiant Soul",
        classNameFr: "Lumièle",
        jobStage: "job3",
        title: "La 2e spécialisation de Laby raconte la belle amitié d'une fillette et de son miroir.",
        path: ["Bébé paillette", "Auralice", "Lumièle"],
        presentation: "Désintéressée et pleine d'énergie, Laby tire ses pouvoirs de sa relation solide avec Nisha." +
            " Elle attire les adversaires à elle pour ensuite les attaquer avec des jets d'eau multicolore, une pluie d'étoiles scintillantes ou des feux d'artifice fabuleux.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "laby",
        pathName: "Nisha Labyrinth",
        pathNameFr: "Labyrinthe de Nisha",
        className: "Nisha Labyrinth",
        classNameFr: "Labyrinthe de Nisha",
        jobStage: "job3",
        title: "La 3e voie de spécialisation de Laby",
        path: ["Bébé obscur", "Rêveuse diurne", "Labyrinthe de Nisha"],
        presentation: "Laby se replie de plus en plus sur elle-même et demande de l'aide à son miroir Nisha. " +
            "Pour protéger Laby, Nisha décide de prendre le contrôle de toutes ses émotions.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }

    },
    {
        characterId: "laby",
        pathName: "Twins Picaro",
        pathNameFr: "Arlequine",
        className: "Twins Picaro",
        classNameFr: "Arlequine",
        jobStage: "job3",
        title: "Laby 4e voie : Arlequine",
        path: ["Morveuse", "Filoute", "Arlequine"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }

    },

    /*Noah*/
    {
        characterId: "noah",
        pathName: "Liberator",
        pathNameFr: "Libérator",
        className: "Liberator",
        classNameFr: "Libérator",
        jobStage: "job3",
        title: "1er voie de Noah",
        path: ["Vengeur d'Âmes", "Ombre Silencieuse", "Libérator"],
        presentation: "",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "noah",
        pathName: "Celestia",
        pathNameFr: "Célestia",
        className: "Celestia",
        classNameFr: "Célestia",
        jobStage: "job3",
        title: "2e voie de Noah",
        path: ["Destinator", "Lanceur d'Étoiles", "Célestia"],
        presentation:
            "Tu actives tes cartes horoscope grâce au cycle et tu te bats selon une stratégie correspondant à ta carte.",
        stats: {
            speed: "Rapide",
            range: "Longue",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "noah",
        pathName: "",
        pathNameFr: "Nyx Pieta",
        className: "",
        classNameFr: "Nyx Pieta",
        jobStage: "job3",
        title: "3e voie de Noah",
        path: ["Lune soucieuse", "Pèlerin lunaire", "Nyx Pieta"],
        presentation:
            "Tu utilises des pierres de lune ainsi que des reliques de la pénitence et du silence pour protéger tes alliés.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }

    },
    {
        characterId: "noah",
        pathName: "",
        pathNameFr: "Morphée",
        className: "",
        classNameFr: "Morphée",
        jobStage: "job3",
        title: "La 4e voie de Noah: Un voyageur onirique qui fuit la réalité",
        path: ["Errelune", "Incandesonge", "Morphée"],
        presentation:
            "À l'heure de la lune sanglante, tu nourris des souvenirs flous et accomplis ta vengeance.",
        stats: {
            speed: "Moyenne",
            range: "Longue",
            difficulty: "Difficile",
            pictureType: "/Images/Combat/Magical_Class.Png",
            attackType: "Attaque Magique",
        }
    },

    /*Lithia*/

    {
        characterId: "lithia",
        pathName: "Gembliss",
        pathNameFr: "Eméraldia",
        className: "Gembliss",
        classNameFr: "Eméraldia",
        jobStage: "job3",
        title: "La première voie de Lithia rejoint le jeu.",
        path: ["Minéralista", "Cristalina", "Eméraldia"],
        presentation: "Elle utilise la force des minerais et combine sa pioche magique avec des reliques pour infliger des dégâts accrus.",
        stats: {
            speed: "Rapide",
            range: "Courte",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "lithia",
        pathName: "Avarice",
        pathNameFr: "Avaricia",
        className: "Avarice",
        classNameFr: "Avaricia",
        jobStage: "job3",
        title: "Elle accorde des avantages spéciaux à ceux à qui elle a donné sa parole. Il s'agit de la 2e spécialisation de Lithia.",
        path: ["Jadoyante", "Adamantine", "Avaricia"],
        presentation: "Avec le pouvoir du minerai doré et sa force de volonté, Lithia renforce son pouvoir magique et soutient ses camarades.",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Difficile",
            pictureType: "/images/combat/Physical_Class.png",
            attackType: "Attaque Physique",
        }
    },
    {
        characterId: "lithia",
        pathName: "Achlys",
        pathNameFr: "Achlys",
        className: "Achlys",
        classNameFr: "Achlys",
        jobStage: "job3",
        title: "Lithia 3e voie : Achlys",
        path: ["Détraqueuse", "Rima Clavis", "Achlys"],
        presentation: "",
        stats: {
            speed: "Moyenne",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }
    },
    {
        characterId: "lithia",
        pathName: "Mischief",
        pathNameFr: "Delinquenta",
        className: "Mischief",
        classNameFr: "Delinquenta",
        jobStage: "job3",
        title: "La quatrième voie de Lithia rejoint le jeu.",
        path: ["Cupido Girl", "Cynical Girl", "Delinquenta"],
        presentation: "Lithia utilise et perfectionne des sorts démoniaques pour attirer ses adversaires et les exterminer impitoyablement.",
        stats: {
            speed: "Rapide",
            range: "Moyenne",
            difficulty: "Moyenne",
            pictureType: "/images/combat/Magical_Class.png",
            attackType: "Attaque Magique",
        }

    },
];