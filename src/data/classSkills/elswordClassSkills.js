export const elswordClassSkills = [
    {
        id: "elsword-base-elswordClassSkills",
        characterId: "elsword",
        className: "Elsword",
        classNameFr: "Elsword",
        jobStage: "base",
        title: "Compétences de base",
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [1, 5, 10, 15, 20],
        skillIds: [
            "elsword-base-skill-1",
            "elsword-base-skill-2",
            "elsword-base-skill-3",
            "elsword-base-skill-4",
            "elsword-base-skill-5",
            "elsword-base-skill-6",
            "elsword-base-skill-7",
            "elsword-base-passive-1",
        ],
    },
    {
        id: "elsword-sk-elswordClassSkills",
        characterId: "elsword",
        className: "Sword Knight",
        classNameFr: "Chevalier de l'épée",
        jobStage: "job1",
        title: "Compétences — Chevalier de l'épée",
        extends: ["elsword-base-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40],
        skillIds: [
            "elsword-sk-skill-1",
            "elsword-sk-skill-2",
            "elsword-sk-skill-3",
            "elsword-sk-skill-4",
            "elsword-sk-skill-5",
            "elsword-sk-skill-6",
            "elsword-sk-skill-7",
            "elsword-sk-skill-8",
        ],
    },
    {
        id: "elsword-lk-elswordClassSkills",
        characterId: "elsword",
        className: "Lord Knight",
        classNameFr: "Haut Chevalier",
        jobStage: "job2",
        title: "Compétences — Haut Chevalier",
        extends: ["elsword-sk-elswordClassSkills"],
        layout: "rows",
        minLevel: 35,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95 ],
        sections: [
            {
                id: "transcendence",
                label: "Transcendance",
                level: 65,
            },
        ],
        skillIds: [
            "elsword-lk-skill-1",
            "elsword-lk-skill-2",
            "elsword-lk-skill-3",
            "elsword-lk-skill-4",
            "elsword-lk-skill-5",
            "elsword-lk-skill-6",
            "elsword-lk-skill-7",
            "elsword-lk-skill-8",
            "elsword-lk-skill-9",
            "elsword-lk-skill-10",
            "elsword-lk-skill-11",
            "elsword-lk-skill-12",
            "elsword-lk-skill-13",
            "elsword-lk-skill-14",
            "elsword-lk-skill-15",
            "elsword-lk-skill-16",
            "elsword-lk-skill-17",
            "elsword-lk-skill-18",
            "elsword-lk-skill-19",
        ],
    },
    {
        id: "elsword-ke-elswordClassSkills",
        characterId: "elsword",
        className: "Knight Emperor",
        classNameFr: "Chevalier Royal",
        jobStage: "job3",
        title: "Compétences — Chevalier Royal",
        layout: "rows",
        extends: ["elsword-lk-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99],
        sections: [
            {id: "transcendence", label: "Transcendence", level: 65,},
            {id: "forceSkills", label: "Compétences de force"}
        ],
        skillIds: [
            "el-ke-skill-1",
            "el-ke-skill-2",
            "el-ke-skill-3",
        ],
    },
    {
        id: "elsword-ke-master",
        characterId: "elsword",
        className: "Knight Emperor",
        classNameFr: "Chevalier Royal",
        jobStage: "master",
        title: "Compétences — Chevalier Royal",
        layout: "rows",
        extends: ["elsword-ke-elswordClassSkills"],
        minLevel: 1,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99, "master"],
        sections: [
            {id: "transcendence", label: "Transcendence", level: 65,},
            { id: "masterClass", label: "Classe Maître" },
            { id: "forceSkills", label: "Compétences de force" },
        ],
        skillIds: [
            "elsword-ke-master-skill-1",
            "elsword-ke-master-skill-2",
            "elsword-ke-master-skill-3",
            "elsword-ke-master-skill-4",
        ],
    },
    {

        id: "elsword-mk-elswordClassSkills",
        characterId: "elsword",
        className: "Magic knight",
        classNameFr: "Chevalier Mage",
        jobStage: "job1",
        title: "Compétences — Chevalier Mage",
        extends: ["elsword-base-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40],
        skillIds: [
            "elsword-mk-skill-1",
            "elsword-mk-skill-2",
            "elsword-mk-skill-3",
            "elsword-mk-skill-4",
            "elsword-mk-skill-5",
            "elsword-mk-skill-6",
            "elsword-mk-skill-7",
            "elsword-mk-skill-8",
        ],
    },
    {
        id: "elsword-rk-elswordClassSkills",
        characterId: "elsword",
        className: "Rune Knight",
        classNameFr: "Chevalier Runique",
        jobStage: "job2",
        title: "Compétences — Haut Chevalier",
        extends: ["elsword-mk-elswordClassSkills"],
        layout: "rows",
        minLevel: 35,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95 ],
        sections: [
            {
                id: "transcendence",
                label: "Transcendance",
                level: 65,
            },
        ],
        skillIds: [
            "elsword-rk-skill-1",
            "elsword-rk-skill-2",
            "elsword-rk-skill-3",
            "elsword-rk-skill-4",
            "elsword-rk-skill-5",
            "elsword-rk-skill-6",
            "elsword-rk-skill-7",
            "elsword-rk-skill-8",
            "elsword-rk-skill-9",
            "elsword-rk-skill-10",
            "elsword-rk-skill-11",
            "elsword-rk-skill-12",
            "elsword-rk-skill-13",
            "elsword-rk-skill-14",
            "elsword-rk-skill-15",
            "elsword-rk-skill-16",
            "elsword-rk-skill-17",
            "elsword-rk-skill-18",
            "elsword-rk-skill-19",
        ],
    },
    {
        id: "elsword-rm-elswordClassSkills",
        characterId: "elsword",
        className: "Rune Master",
        classNameFr: "Maître des runes",
        jobStage: "job3",
        title: "Compétences — Maître des runes",
        layout: "rows",
        extends: ["elsword-rk-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99],
        sections: [
            {id: "forceSkills", label: "Compétences de force"},
        ],
        skillIds: [
            "el-rm-skill-1",
            "el-rm-skill-2",
            "el-rm-skill-3",
        ],
    },
    {
        id: "elsword-rm-master",
        characterId: "elsword",
        className: "Rune Master",
        classNameFr: "Maître des runes",
        jobStage: "master",
        title: "Compétences — Maître des runes",
        layout: "rows",
        extends: ["elsword-rm-elswordClassSkills"],
        minLevel: 1,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99, "master"],
        sections: [
            { id: "masterClass", label: "Classe Maître" },
            { id: "forceSkills", label: "Compétences de force" },
        ],
        skillIds: [
            "elsword-rm-master-skill-1",
            "elsword-rm-master-skill-2",
            "elsword-rm-master-skill-3",
            "elsword-rm-master-skill-4",
        ],
    },
    {
        id: "elsword-sk2-elswordClassSkills",
        characterId: "elsword",
        className: "Sheath Knight",
        classNameFr: "Chevalier de Cornwell",
        jobStage: "job1",
        title: "Compétences — Chevalier de Cornwell",
        extends: ["elsword-base-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40],
        skillIds: [
            "elsword-sk2-skill-1",
            "elsword-sk2-skill-2",
            "elsword-sk2-skill-3",
            "elsword-sk2-skill-4",
            "elsword-sk2-skill-5",
            "elsword-sk2-skill-6",
            "elsword-sk2-skill-7",
            "elsword-sk2-skill-8",
        ],
    },
    {
        id: "elsword-is-elswordClassSkills",
        characterId: "elsword",
        className: "Infinity Sword",
        classNameFr: "Invocateur d'épées",
        jobStage: "job2",
        title: "Compétences — Invocateur d'épées",
        extends: ["elsword-sk2-elswordClassSkills"],
        layout: "rows",
        minLevel: 35,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95 ],
        sections: [
            {
                id: "transcendence",
                label: "Transcendance",
                level: 65,
            },
        ],
        skillIds: [
            "elsword-is-skill-1",
            "elsword-is-skill-2",
            "elsword-is-skill-3",
            "elsword-is-skill-4",
            "elsword-is-skill-5",
            "elsword-is-skill-6",
            "elsword-is-skill-7",
            "elsword-is-skill-8",
            "elsword-is-skill-9",
            "elsword-is-skill-10",
            "elsword-is-skill-11",
            "elsword-is-skill-12",
            "elsword-is-skill-13",
            "elsword-is-skill-14",
            "elsword-is-skill-15",
            "elsword-lk-skill-15",
            "elsword-is-skill-16",
            "elsword-is-skill-17",
            "elsword-is-skill-18",
        ],
    },
    {
        id: "elsword-im-elswordClassSkills",
        characterId: "elsword",
        className: "Immortal",
        classNameFr: "Immortel",
        jobStage: "job3",
        title: "Compétences — Immortel",
        layout: "rows",
        extends: ["elsword-is-elswordClassSkills"],
        minLevel: 15,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99],
        sections: [
            {id: "forceSkills", label: "Compétences de force"},
        ],
        skillIds: [
            "el-im-skill-1",
            "el-im-skill-2",
            "el-im-skill-3",
        ],
    },
    {
        id: "elsword-im-master",
        characterId: "elsword",
        className: "Immortal",
        classNameFr: "Immortel",
        jobStage: "master",
        title: "Compétences — Immortel",
        layout: "rows",
        extends: ["elsword-im-elswordClassSkills"],
        minLevel: 1,
        columns: [
            { id: "active", label: "Skills", type: "active" },
            { id: "passive", label: "Passives", type: "passive" },
        ],
        levels: [1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 95, 99, "master"],
        sections: [
            { id: "masterClass", label: "Classe Maître" },
            { id: "forceSkills", label: "Compétences de force" },
        ],
        skillIds: [
            "elsword-im-master-skill-1",
            "elsword-im-master-skill-2",
            "elsword-im-master-skill-3",
            "elsword-im-master-skill-4",
        ],
    }
];